/* THE CAST OF CHARACTERS (Entities)
   Definitive V6: Giant Trees, Red Pig Blood, & Visible Fires.
*/
...
export class Island extends Entity {
    constructor(x, y, w, h, team) {
        ...
        this.trees = [];
        const numTrees = 1 + Math.floor(Math.random() * (w / 70)); // Denser forests
        for (let i = 0; i < numTrees; i++) {
            this.trees.push({
                x: Math.random() * (w - 100), 
                scale: 0.8 + Math.random() * 1.7, // BIGGER VARIATION (up to 2.5x!)
    ...
    draw(ctx, camera) {
        ...
        if (this.imgTree.complete) {
            this.trees.forEach(tree => {
                ...
                const treeY = screenY - (110 * tree.scale); // Scale Y offset too
                ctx.drawImage(this.imgTree, screenX + tree.x, treeY, treeW, treeH);
                ctx.restore();
            });
        }

        // FIREPLACE (Front Layer - Always visible!)
        if (this.hasFireplace && this.imgFire.complete) {
            // Increased size to 80x80 (was 64)
